<form role="form" method="POST" action="{{ url('/users' . ($user->id ? "/{$user->id}" : '')) }}" autocomplete="off">
    @csrf
    @if($user->id)
    @method('put')
    @endif
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-sm-12 col-md-6">
                ...
                <div class="input-group input-group-sm mb-2">
                    <select id="is_admin"
                        class="form-control-sm {{ $errors->has('is_admin') ? 'is-invalid' : '' }} select2"
                        name="is_admin">
            ...
            <div class="col-sm-12 col-md-6">
                ...
                <div class="input-group mb-2">
                    <select id="role" class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }} not-select"
                        ...
                        @foreach($roles as $r)
                        <option value="{{$r->id}}"
                            {{$user->hasRole($r) || in_array($r->id,old('role',[])) ? 'selected' : ''}}>
                            {{ucwords($r->name)}}
                        </option>
                        @endforeach
                    </select>
